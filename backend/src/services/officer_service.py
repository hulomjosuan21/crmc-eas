...
class OfficerService:
    ...
    async def bind_google_credentials(self, officer_id: uuid.UUID, user_info: dict) -> Officer:
        ...
        officer.oauth_email = user_info.get('email')
        officer.oauth_image = user_info.get('picture')

        await self.db.commit()
        await self.db.refresh(officer)
        return officer

    async def delete_pending_officer(self, officer_id: uuid.UUID):
        officer = await self.repo.get_by_id(officer_id)
